...
import { ArrowRight, Building2, HeartHandshake, LoaderCircle, LockKeyhole, UserRound, UsersRound } from 'lucide-react';
    ...
    <section className="login-panel">
      <div className="login-card">
        ...
        <div className="auth-tabs">
          <button type="button" onClick={() => { setMode('signin'); setError(''); }} className={mode === 'signin' ? 'auth-tab active' : 'auth-tab'}>Sign In</button>
          <button type="button" onClick={() => { setMode('signup'); setError(''); }} className={mode === 'signup' ? 'auth-tab active' : 'auth-tab'}>Create Account (Sign Up)</button>
        </div>

        <div className="login-heading">
          <p className="eyebrow">{mode === 'signin' ? 'WELCOME BACK' : 'GET STARTED'}</p>
          <h2>{mode === 'signin' ? 'Choose your workspace.' : 'Create your profile.'}</h2>
          <p>{mode === 'signin' ? 'Use the role your account was registered with.' : 'Fill in your details according to your selected role.'}</p>
        </div>
        ...
        <p className="login-help">
          {mode === 'signin' ? (
            <>New to TrackAthlete? <button type="button" onClick={() => { setMode('signup'); setError(''); }} style={{ background: 'none', border: 'none', color: '#e07050', fontWeight: 'bold', cursor: 'pointer', textDecoration: 'underline' }}>Click here to Sign Up</button></>
          ) : (
            <>Already have an account? <button type="button" onClick={() => { setMode('signin'); setError(''); }} style={{ background: 'none', border: 'none', color: '#e07050', fontWeight: 'bold', cursor: 'pointer', textDecoration: 'underline' }}>Click here to Sign In</button></>
          )}
        </p>
